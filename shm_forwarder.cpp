...
int main(int argc, char** argv) {
    if (argc < 5 || argc > 7) {
        std::cerr
            << "Usage: shm_xdp_forwarder <shm_channel> <shm_stream_id> "
               "<dest_addr> <dest_port> [src_port] [xdp_iface]\n"
            << "Example: shm_xdp_forwarder myapp 1001 "
    ...
    try {
        ...
        ShmIpcSource  source(channel, streamId);

        std::cout << "[shm-forwarder] shm:" << detail::shmName(channel, streamId)
                  << " -> " << destAddr << ":" << destPort
                  << " (Ctrl-C to stop)\n";

        source.run([&](const uint8_t* data, uint16_t len) {
            if (!sink.send(data, len))
                std::cerr << "[shm-forwarder] send failed\n";
        });
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "[shm-forwarder] fatal: " << e.what() << "\n";
        return 1;
    }
}
